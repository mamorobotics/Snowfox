...
extern "C" {
	...
	void cv_float16_t_fromBits_unsigned_short(unsigned short w, Result<cv::float16_t*>* ocvrs_return) {
		try {
			...
			Ok(new cv::float16_t(ret), ocvrs_return);
		} OCVRS_CATCH(ocvrs_return);
	}
	...
	cv::float16_t* cv_float16_t_implicitClone_const(const cv::float16_t* instance) {
			return new cv::float16_t(*instance);
	}
	
	void cv_float16_t_delete(cv::float16_t* instance) {
			delete instance;
	}
