pub mod gapi {
	...
	pub trait GKernelPackageTraitConst {
		...
		/// Returns total number of kernels
		...
		fn size(&self) -> Result<size_t> {
			return_send!(via ocvrs_return);
			unsafe { sys::cv_GKernelPackage_size_const(self.as_raw_GKernelPackage(), ocvrs_return.as_mut_ptr()) };
			return_receive!(unsafe ocvrs_return => ret);
			let ret = ret.into_result()?;
			Ok(ret)
		}
	...
	pub trait GBackendTraitConst {
		...
		fn hash(&self) -> Result<size_t> {
			return_send!(via ocvrs_return);
			unsafe { sys::cv_gapi_GBackend_hash_const(self.as_raw_GBackend(), ocvrs_return.as_mut_ptr()) };
			return_receive!(unsafe ocvrs_return => ret);
			let ret = ret.into_result()?;
			Ok(ret)
		}
