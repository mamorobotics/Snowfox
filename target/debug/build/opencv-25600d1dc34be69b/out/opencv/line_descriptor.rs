pub mod line_descriptor {
	...
	//! image, we first compute a non-binary descriptor as described in [LBD](https://docs.opencv.org/4.9.0/d0/de3/citelist.html#CITEREF_LBD) . Such algorithm works on
	//! lines extracted using EDLine detector, as explained in [EDL](https://docs.opencv.org/4.9.0/d0/de3/citelist.html#CITEREF_EDL) . Given a line, we consider a
	...
	/// -------------------
	/// 
	/// The theory described in this section is based on [MIH](https://docs.opencv.org/4.9.0/d0/de3/citelist.html#CITEREF_MIH) . Given a dataset populated with binary
	...
	impl Drop for DrawLinesMatchesFlags {
		#[inline]
		fn drop(&mut self) {
			unsafe { sys::cv_line_descriptor_DrawLinesMatchesFlags_delete(self.as_raw_mut_DrawLinesMatchesFlags()) };
		}
	}
	...
	boxed_ref! { DrawLinesMatchesFlags, crate::line_descriptor::DrawLinesMatchesFlagsTraitConst, as_raw_DrawLinesMatchesFlags, crate::line_descriptor::DrawLinesMatchesFlagsTrait, as_raw_mut_DrawLinesMatchesFlags }
	
	impl DrawLinesMatchesFlags {
		/// Creates a default instance of the class by calling the default constructor
		#[inline]
		fn default() -> Self {
			unsafe { Self::from_raw(sys::cv_line_descriptor_DrawLinesMatchesFlags_defaultNew_const()) }
		}
		
	}
	
	impl Clone for DrawLinesMatchesFlags {
		#[inline]
		fn clone(&self) -> Self {
			unsafe { Self::from_raw(sys::cv_line_descriptor_DrawLinesMatchesFlags_implicitClone_const(self.as_raw_DrawLinesMatchesFlags())) }
		}
	}
	
	impl std::fmt::Debug for DrawLinesMatchesFlags {
		#[inline]
		fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
			f.debug_struct("DrawLinesMatchesFlags")
				.finish()
		}
	}
	
	impl Default for DrawLinesMatchesFlags {
		#[inline]
	...
	/// ----------------------------
	/// 
	/// The lines extraction methodology described in the following is mainly based on [EDL](https://docs.opencv.org/4.9.0/d0/de3/citelist.html#CITEREF_EDL) . The
